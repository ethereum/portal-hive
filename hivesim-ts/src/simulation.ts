...
import { TestMatcher } from './testmatch.js'
import { ClientDefinition, StartNodeResponse, SuiteID, TestID, TestRequest } from './types.js'
export class Simulation {
  ...
  async start_test(test_suite: SuiteID, name: string, description: string): Promise<SuiteID> {
    const url = this.url + '/testsuite/' + test_suite + '/test'
    const body: TestRequest = { name, description }
  ...
  async end_test(test: Test) {
    const url = this.url + '/testsuite/' + test.suite_id + '/test/' + test.test_id
    const request = {
      method: 'POST',
  ...
  async start_client(
    ...
    const url = this.url + '/testsuite/' + test_suite + '/test/' + test + '/node'
