...
const two_client_demo = async (
  ...
  const ping1 = await client_a.rpc.request("portal_historyPing", [
    nodeInfo2.enr,
  ]);
  const ping2 = await client_b.rpc.request("portal_historyPing", [
    nodeInfo1.enr,
  ]);
  console.log({
    nodeInfo1,
...
const n_client_demo = async (test: Test, clients: IClient[]) => {
  ...
  const routingTables = await Promise.all(
    clients.map(async (client) => {
      const res = await client.rpc.request(
